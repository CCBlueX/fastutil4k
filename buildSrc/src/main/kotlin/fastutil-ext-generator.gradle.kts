    ...
    content {
        ...
        forEachPrimitiveTypes { type ->
            for ((prefix, fullType) in prefixToFullType) {
                ...
                if (type != FastutilType.BOOLEAN && (prefix == "RBTree" || prefix == "AVLTree" || prefix == "Linked") || (prefix == "Array" || prefix == "Hash")) {
...
val enumMapTask = tasks.register<GenerateSrcTask>("enum-map") {
    ...
    content {
        ...
        withIndent {
            appendLine("val m = EnumMap<K, V>(K::class.java)")
            appendLine("for (k in K::class.java.enumConstants) {")
            withIndent {
                appendLine("m.put(k, mapping(k))")
            }
            appendLine("}")
            appendLine("return m")
        }
        ...
        for (entryParam in arrayOf("vararg entries: K", "entries: Iterable<K>")) {
            appendLine("inline fun $generic enumMapOf($entryParam, mapping: (K) -> V): EnumMap<K, V> {")
            withIndent {
                appendLine("val m = EnumMap<K, V>(K::class.java)")
                appendLine("for (k in entries) {")
                withIndent {
                    appendLine("m.put(k, mapping(k))")
                }
                appendLine("}")
